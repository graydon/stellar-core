...
class BucketList
{
  ...
  public:
    ...
    // Pre-protocol-11 version of same, taking only undifferentiated-live and
    // dead entries. Forwards to the post-protocol-11 variant above. For testing
    // only, should be eliminated eventually.
    void addBatch(Application& app, uint32_t currLedger,
                  std::vector<LedgerEntry> const& liveEntries,
                  std::vector<LedgerKey> const& deadEntries);
};
