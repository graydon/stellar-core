...
TEST_CASE("xdr siphash", "[siphash]")
{
    for (size_t i = 0; i < 1000; ++i)
    {
        auto entry = stellar::LedgerTestUtils::generateValidLedgerEntry(100);
        auto bytes_hash =
            stellar::shortHash::computeHash(xdr::xdr_to_opaque(entry));
        auto stream_hash = stellar::shortHash::computeXDRHash(entry);
