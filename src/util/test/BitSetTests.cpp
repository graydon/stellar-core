...
TEST_CASE("BitSet union", "[bitset]")
{
    for (size_t loop = 0; loop < 100; ++loop) {
        ...
        BitSet bs_a, bs_b;
        for (size_t i = 0; i < rand_uniform<size_t>(size_t(1), size_t(100)); ++i)
        {
            ref.insert(i);
            bs_a.set(i);
        }
        ...
        REQUIRE(bs_a.union_count(bs_b) == ref.size());
        for (size_t i = 0; bs_c.next_set(i); ++i)
        {
...
TEST_CASE("BitSet intersection", "[bitset]")
{
    for (size_t loop = 0; loop < 100; ++loop) {
        ...
        BitSet bs_c = bs_a & bs_b;
        REQUIRE(bs_a.intersection_count(bs_b) == ref.size());
        for (size_t i = 0; bs_c.next_set(i); ++i)
        {
            REQUIRE(ref.find(i) != ref.end());
        }
...
TEST_CASE("BitSet difference", "[bitset]")
{
    for (size_t loop = 0; loop < 100; ++loop) {
        ...
        BitSet bs_c = bs_a - bs_b;
        REQUIRE(bs_a.difference_count(bs_b) == ref.size());
        for (size_t i = 0; bs_c.next_set(i); ++i)
        {
            REQUIRE(ref.find(i) != ref.end());
        }
...
TEST_CASE("BitSet symmetric difference", "[bitset]")
{
    for (size_t loop = 0; loop < 100; ++loop) {
        ...
        BitSet bs_c = bs_a.symmetric_difference(bs_b);
        REQUIRE(bs_a.symmetric_difference_count(bs_b) == ref.size());
        for (size_t i = 0; bs_c.next_set(i); ++i)
        {
            REQUIRE(ref.find(i) != ref.end());
        }
