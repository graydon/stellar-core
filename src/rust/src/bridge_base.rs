...
pub(crate) mod rust_bridge {
    ...
    // Lazy-path equivalents of CxxLedgerInfo and CxxRentFeeConfiguration.
    // These use Vec<u8> instead of CxxBuf for byte fields so the lazy
    // invocation path never needs to allocate C++ vectors.
    struct CxxLedgerInfoLazy {
        pub protocol_version: u32,
        pub sequence_number: u32,
        pub timestamp: u64,
        pub network_id: Vec<u8>,
    ...
    #[derive(Debug)]
    struct CxxRentFeeConfigurationLazy {
