...
int
runReplayLedgerForStreaming(CommandLineArgs const& args)
{
    CommandLine::ConfigOption configOption;
    std::string catchupString;
...
handleCommandLine(int argc, char* const* argv)
{
    auto commandLine = CommandLine{
        {{"catchup",
         ...
         {"replay-ledger-for-streaming",
          "replay a ledger range from history, streaming changes",
          runReplayLedgerForStreaming},
