...
class Config : public std::enable_shared_from_this<Config>
{
  ...
  public:
    ...
    // A string representing an _existing_ stream to write fine-grained
    // metadata to for each ledger close while running. This will be opened at
    // startup and synchronously streamed-to during both catchup and live
    // ledger-closing.
    //
    // Streams may be specified either as an existing pathname (typically a
    // named FIFO on POSIX or a named pipe on Windows) or a string of the form
    // "fd:N" for some integer N which, on POSIX, specifies the existing open
    // file descriptor N inherited by the process (for example to write to an
    // anonymous pipe). Anonymous file descriptor streams are not supported on
    // Windows (anonymous pipes do not work with the ASIO library used in
    // stellar-core).
    //
    ...
    // delay in closing a ledger, and should not be
    // used on a node participating in consensus, only a passive "watcher" node.
