...
TEST_CASE("quorum intersection basic 4-node", "[scp][quorumintersection]")
{
    ...
    CHECK(qic->networkEnjoysQuorumIntersection());
}
...
generate_orgs(size_t n_orgs, std::vector<size_t> sizes = {3, 5})
{
          ...
          "[scp][quorumintersection]")
{
    ...
    //    org6           org7
    //
    auto orgs = generate_orgs(8, {3, 3, 3, 3, 2, 2, 2, 2});
...
TEST_CASE("quorum intersection 6-org 1-node 4-wildcard",
           "[scp][quorumintersection]")
{
    ...
    // "self + 3 neighbours"); the quorum intersection checker then adjust them
    // for missing information by reducing all their thresholds by 1 for each
    // unknown node they depend on, resulting in a 1-of-2 threshold for org0 and
    // org1, causing the topology to split.

    auto orgs = generate_orgs(6, {1,1,1,1,1,1});
    auto qm = interconnectOrgsUnidir(orgs, {
            {0, 1}, {1, 0},
            {0, 2}, {0, 3},
            {1, 4}, {1, 5},
