...
void
LedgerTxn::dropConfig()
{
    throw std::runtime_error("called dropConfig on non-root LedgerTxn");
}
#endif
...
BulkLedgerEntryChangeAccumulator::accumulate(EntryIterator const& iter)
{
    ...
    case CONTRACT_DATA:
        ...
        break;
    case CONFIG:
        accum(iter, mConfigToUpsert, mConfigToDelete);
        break;
#endif
    default:
        abort();
...
LedgerTxnRoot::Impl::prefetch(UnorderedSet<LedgerKey> const& keys)
{
    ...
    for (auto const& key : keys)
    {
        ...
        case CONTRACT_DATA:
            ...
            break;
        case CONFIG:
            insertIfNotLoaded(config, key);
            if (config.size() == mBulkLoadBatchSize)
            {
