...
static void
throwIfNotConfig(LedgerEntryType type)
{
    if (type != CONFIG)
    {
        throw NonSociRelatedException("LedgerEntry is not a CONFIG");
    }
}

std::shared_ptr<LedgerEntry const>
LedgerTxnRoot::Impl::loadConfig(LedgerKey const& key) const
{
    int32_t configID = key.config().configID;
    std::string configEntryStr;
    ...
    st.exchange(soci::into(configEntryStr));
    st.exchange(soci::use(configID));
    st.define_and_bind();
    {
        auto timer = mDatabase.getSelectTimer("config");
        st.execute(true);
    }
    if (!st.got_data())
    {
    ...
    LedgerEntry le;
    fromOpaqueBase64(le, configEntryStr);
    throwIfNotConfig(le.data.type());

    return std::make_shared<LedgerEntry const>(std::move(le));
}

class BulkLoadConfigOperation
    : public DatabaseTypeSpecificOperation<std::vector<LedgerEntry>>
{
    ...
    executeAndFetch(soci::statement& st)
    {
        ...
        st.define_and_bind();
        {
            auto timer = mDb.getSelectTimer("config");
            st.execute(true);
        }
  ...
  public:
    BulkLoadConfigOperation(Database& db, UnorderedSet<LedgerKey> const& keys)
        : mDb(db)
    {
        mConfigSettingIDs.reserve(keys.size());
        for (auto const& k : keys)
        {
            throwIfNotConfig(k.type());
            mConfigSettingIDs.emplace_back(k.config().configID);
        }
    }
...
LedgerTxnRoot::Impl::bulkLoadConfig(UnorderedSet<LedgerKey> const& keys) const
{
    if (!keys.empty())
    {
        BulkLoadConfigOperation op(mDatabase, keys);
...
class BulkDeleteConfigOperation : public DatabaseTypeSpecificOperation<void>
{
    Database& mDb;
  ...
  public:
    BulkDeleteConfigOperation(Database& db, LedgerTxnConsistency cons,
        ...
        : mDb(db), mCons(cons)
    {
        ...
        for (auto const& e : entries)
        {
            ...
            throwIfNotConfig(e.key().ledgerKey().type());
            mConfigSettingIDs.emplace_back(
                e.key().ledgerKey().config().configID);
        }
    }
    ...
    {
        std::string sql = "DELETE FROM config WHERE configsettingid = :id";
        auto prep = mDb.getPreparedStatement(sql);
        auto& st = prep.statement();
        st.exchange(soci::use(mConfigSettingIDs));
        st.define_and_bind();
        {
            auto timer = mDb.getDeleteTimer("config");
            st.execute(true);
        }
...
void
LedgerTxnRoot::Impl::bulkDeleteConfig(std::vector<EntryIterator> const& entries,
                                      LedgerTxnConsistency cons)
{
    BulkDeleteConfigOperation op(mDatabase, cons, entries);
    mDatabase.doDatabaseTypeSpecificOperation(op);
}

class BulkUpsertConfigOperation : public DatabaseTypeSpecificOperation<void>
{
    Database& mDb;
    std::vector<int32_t> mConfigSettingIDs;
    std::vector<std::string> mConfigEntries;
    std::vector<int32_t> mLastModifieds;

    void
    accumulateEntry(LedgerEntry const& entry)
    {
        throwIfNotConfig(entry.data.type());

        mConfigSettingIDs.emplace_back(entry.data.config().configID);
        mConfigEntries.emplace_back(toOpaqueBase64(entry));
        mLastModifieds.emplace_back(
            unsignedToSigned(entry.lastModifiedLedgerSeq));
    }

  public:
    BulkUpsertConfigOperation(Database& Db,
    ...
    doSociGenericOperation()
    {
        std::string sql = "INSERT INTO config "
        ...
        st.exchange(soci::use(mConfigSettingIDs));
        st.exchange(soci::use(mConfigEntries));
        st.exchange(soci::use(mLastModifieds));
        st.define_and_bind();
        {
            auto timer = mDb.getUpsertTimer("config");
            st.execute(true);
        }
        if (static_cast<size_t>(st.get_affected_rows()) !=
            mConfigSettingIDs.size())
    ...
    doPostgresSpecificOperation(soci::postgresql_session_backend* pg) override
    {
        std::string strConfigSettingIDs, strConfigEntries, strLastModifieds;

        PGconn* conn = pg->conn_;
        marshalToPGArray(conn, strConfigSettingIDs, mConfigSettingIDs);
        marshalToPGArray(conn, strConfigEntries, mConfigEntries);
        ...
        soci::statement& st = prep.statement();
        st.exchange(soci::use(strConfigSettingIDs));
        st.exchange(soci::use(strConfigEntries));
...
void
LedgerTxnRoot::Impl::bulkUpsertConfig(std::vector<EntryIterator> const& entries)
{
    BulkUpsertConfigOperation op(mDatabase, entries);
    mDatabase.doDatabaseTypeSpecificOperation(op);
}

void
LedgerTxnRoot::Impl::dropConfig()
{
    throwIfChild();
    mEntryCache.clear();
    mBestOffers.clear();
    ...
    mDatabase.getSession() << "DROP TABLE IF EXISTS config;";
    mDatabase.getSession() << "CREATE TABLE config ("
