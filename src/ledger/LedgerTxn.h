...
class AbstractLedgerTxnParent
{
  public:
    ...
    virtual uint64_t countObjects(LedgerEntryType let) const { return 0; }
    virtual uint64_t countObjects(LedgerEntryType let,
                                  LedgerRange const& ledgers) const { return 0; }

    virtual void deleteObjectsModifiedOnOrAfterLedger(uint32_t ledger) const {}

    virtual void dropAccounts() {}
    virtual void dropData() {}
    virtual void dropOffers() {}
    virtual void dropTrustLines() {}

    virtual double getPrefetchHitRate() const { return 100.0; }
    virtual uint32_t prefetch(std::unordered_set<LedgerKey> const& keys) { return 0; }
